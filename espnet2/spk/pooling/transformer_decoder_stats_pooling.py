...
class TransformerDecoderStatsPooling(AbsPooling, BaseTransformerDecoder):
    """
    ...
    def __init__(
        ...
        self._output_size = attention_dim + (input_size * 2)
    ...
    def forward(
        ...
        """
        # get stats pooling
        mu = encoder_output.mean(dim=2)
        sg = encoder_output.std(dim=2)
        ...
        if self.normalize_before:
            x = self.after_norm(x)
        x = x.squeeze(1) # decoder output embd

        x = torch.cat([x, mu, sg], dim=1)
